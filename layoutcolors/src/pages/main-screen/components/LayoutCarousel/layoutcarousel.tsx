...
const LayoutCarousel = () => {
  ...
  return (
    <div style={{ width: '900px', height: '480px', border: '1px solid #ccc', position: 'relative' }}>
      <button onClick={handlePrev} style={{ position: 'absolute', top: '50%', left: '0px', transform: 'translateY(-50%)' }}><img src="../../images/setaleft" alt="" /></button>
      <div style={{ width: '100%', height: '100%' }}>
        {layouts[currentLayout]}
      </div>
      <button className='buttonleftright' onClick={handleNext} style={{ position: 'absolute', top: '50%', right: '0', transform: 'translateY(-50%)' }}>▶</button>
    </div>
  );
};
